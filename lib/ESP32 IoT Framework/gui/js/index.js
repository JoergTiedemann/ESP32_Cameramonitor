...
function Root() {
    ...
    function wsMessage(event) {
        event.data.arrayBuffer().then((buffer) => {                
            ...
            const dashsize   = dv.getUint32(8, true);
            // als 3. die Laenge des Arrays fuer historische Daten
            const historicArraylength  = dv.getUint32(8+8, true);
            // nun die eigentlichen Dashdaten 
            displayData.push([timestamp, bin2obj(buffer.slice(24,24+dashsize), Dash)]);     
            // nun die historischen Arraydaten  
            if (historicArraylength > 0)
            {
                const ArrElementlength = (buffer.byteLength-24-dashsize) / historicArraylength;
                // erstmal alles loeschen
                historicArrayData.length = 0;
                for (let i = 0; i < historicArraylength; i++) 
                {
                    historicArrayData.push(bin2obj(buffer.slice(24+dashsize+i*ArrElementlength,24+dashsize+(i+1)*ArrElementlength), HistoricData));     
                }
                // console.log("Arrayinhalt");
                //  for (let i = 0; i < historicArraylength; i++) 
                //  {
