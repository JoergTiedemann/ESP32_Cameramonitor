...
export function ControlItem(props) {
   ...
   {
    if ((props.name == "ReloadPicture") && (props.displayType == "button"))
    {
        // 1fach Button
        return <>
        <Button onClick={(e) => {e.preventDefault();location.reload(); }} type={props.type} id={props.conditionalAttributes.buttonIdOn} name={props.name}>{props.conditionalAttributes.buttontextOn}</Button>
        </>;
    }
    else
    {
