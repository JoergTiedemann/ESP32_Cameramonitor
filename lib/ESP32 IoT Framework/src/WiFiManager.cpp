...
{
    WifiManager* pWiFiManager =(WifiManager*)parameter;
    // Serial.printf("Scan start networkscantask\n");
    String str,strenc;
    ...
    while (n < 0)
    {
        vTaskDelay(500);
        n = WiFi.scanComplete();
        // Serial.printf("Result scanComplete %d\n",n);
        if(n) 
        {
        // Serial.printf("Scan done:%d\n",n);
        if(n) {
                pWiFiManager->m_strScanData.free();
                pWiFiManager->m_ScanResult=0;
                for (int i = 0; i < n; ++i){
                    switch (WiFi.encryptionType(i))
                    {
                        ...
                        case WIFI_AUTH_WPA_WPA2_PSK:
                            ...
                            break;
                        default:
                            strenc = "unbekannt:"+String(WiFi.encryptionType(i));
                            break;
                    }
                    String strHide = "";
                    #ifdef ESP8266
                        if (WiFi.isHidden(i))
                            strHide = " SSID versteckt";
                    #endif
                    str = "SSID:"+String(WiFi.SSID(i)) + " " + String(WiFi.RSSI(i)) +("dB Kanal:")+String(WiFi.channel(i)) + " Sicherheit:" + strenc + strHide;
                    //  Serial.printf("Index:%d Result:",i);
                    //  Serial.println(str);
                    pWiFiManager->m_strScanData.add(str);
                    pWiFiManager->m_ScanResult = i+1;
                }
                WiFi.scanDelete();
            }
            else
        ...
        // else {
        //     Serial.printf("Scan Else Result:%d\n",n);
        // }
    }
    pWiFiManager->m_bNetworkScan = false;

    // Serial.printf("Free Heap Size = %d\n", ESP.getFreeHeap());
    // Serial.printf("End Networkscantask\n");
 
    pWiFiManager->m_ScanTaskHandle = NULL;
    vTaskDelete( NULL );   
}
...
void WifiManager::StartNetworkscan()
{
    ...
    {
        m_bNetworkScan = true;
        //xTaskCreate(ScanNetworkTask, "ScanWifiNetworkTask", 4096, this, 1, &m_ScanTaskHandle);
        // der Sketch laeuft normalerweise auf Core 1 das tut aber nicht
        // Core 0 tut auch nicht, solange der Networkscan laeuft ist das netzwerke und damit der Empfang von der Response auf HTTP requests geblockt
        // wir muessen das also mit asynchronem Netzwerkscan probieren
        // wetzerhn haben wir den Effekt das die Connection verloren wird während des scans vielleicht muessen wir das was mit Timeout machen
        xTaskCreatePinnedToCore(ScanNetworkTask, "ScanWifiNetworkTask", 4096, this, 1, &m_ScanTaskHandle,0);
    }
}

String WifiManager::GetScanString(int iElement)
{
    if ((iElement>=0) && (iElement < m_strScanData.length()))
        return *m_strScanData.nth(iElement);
    else
        return String("");
}
